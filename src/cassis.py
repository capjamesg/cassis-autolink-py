...
def auto_link_re():
    """
    A regular expression to add links to a string.
...
def auto_link(text, do_embed=False, max_url_length=0):
    """
    Add HTML links to URLs in the provided text.

    :param text: The text to which links should be added.
    :type text: str
    :param do_embed: If True, support embedding citations with a <blockquote>.
    :type do_embed: bool
    :param max_url_length: If non-zero, truncate URLs longer than this.
    :type max_url_length: int
    :returns: The text with links added.
    :rtype: string
    """
...
def web_address_to_uri(wa, addhttp=False):
    """
    Convert a web address to a URI.

    :param wa: Web address to convert.
    :type wa: str
    :param addhttp: If True, add http:// to the beginning of the URI this function returns
    :type addhttp: bool
